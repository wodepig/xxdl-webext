  ...
  <div class="p-4 w-[470px] min-h-[400px] bg-base-200">
    ...
    <div class="flex items-center justify-between mb-4">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h1 class="text-lg font-bold">新道云</h1>
          <p class="text-xs text-base-content/60">xindaoyun.com</p>
        </div>
      </div>
      <HeaderActions />
    </div>
      ...
      <template #account>
        <div class="bg-base-100 border border-base-300 p-4 rounded-box">
          ...
          <button @click="getAccountData" :disabled="gettingAccount" class="btn btn-primary btn-sm w-full mb-4">
            <svg v-if="!gettingAccount" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div v-if="accountData" class="mb-4">
            <div class="alert alert-success">
              ...
              <div>
                ...
                <div class="text-xs">用户名: <span class="font-mono font-bold text-primary">{{ accountData.username }}</span></div>
              </div>
            </div>
          </div>
          ...
          <div v-if="accountData" class="collapse collapse-arrow bg-base-200">
            ...
            <div class="collapse-content">
              <pre class="bg-base-300 p-3 rounded text-xs overflow-x-auto">{{ JSON.stringify(accountData, null, 2) }}</pre>
            </div>
          </div>
        </div>
      </template>
      ...
      <template #cookie>
        <div class="bg-base-100 border border-base-300 p-4 rounded-box">
          ...
          <button @click="getCookieData" :disabled="gettingCookie" class="btn btn-secondary btn-sm w-full mb-4">
            <svg v-if="!gettingCookie" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div v-if="cookieData" class="collapse collapse-arrow bg-base-200">
            ...
            <div class="collapse-content">
              <pre class="bg-base-300 p-3 rounded text-xs overflow-x-auto">{{ JSON.stringify(cookieData, null, 2) }}</pre>
            </div>
          </div>
        </div>
      </template>
    ...
    <button @click="sendToBackend" :disabled="!canSend || sending" class="btn btn-success w-full">
      <svg v-if="!sending" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
async function getAccountData() {
  ...
  accountData.value = {
    type: 'account',
    username: 'zhangsan_' + Math.floor(Math.random() * 1000),
...
async function sendToBackend() {
  ...
  try {
    // 这里应该调用实际的 API
    // const result = await httpClient.post(`${backendUrl}/api/data`, {
    //   apiKey,
    //   site: 'xindaoyun.com',
    //   data: dataToSend
    // })
    ...
    // 模拟成功
    toastRef.value?.show({ message: `${activeTab.value === 'account' ? '账号' : 'Cookie'}数据发送成功`, type: 'success' })
