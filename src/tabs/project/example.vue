  ...
  <div class="p-4 w-[470px] min-h-[400px] bg-base-200">
    ...
    <div class="flex items-center justify-between mb-4">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h1 class="text-lg font-bold">{{ projectConfig?.name || 'Example' }}</h1>
          <p class="text-xs text-base-content/60">{{ projectConfig?.matchUrl?.join(',') || 'example.com' }}</p>
        </div>
      </div>
      <HeaderActions />
    </div>
    ...
    <!-- 功能内容区域 - Tab 标签页 -->
    <Tabs v-model="activeTab" :tabs="tabs" name="example_tabs" custom-class="mb-4">
      <!-- 账号 Tab -->
      <template #account>
